...
def user_move(message):
    ...
    if game_over:
        ...
        if error:
            ...
            win_combo()
            if game_over:
                bot.send_message(user_id, f"Человек победил!")
...
def ai_move():
    ...
    if max_move > candy:
        max_move = candy
    bot.send_message(
        user_id, f"Можно взять от {min_move} до {max_move} конфет")
    ...
    win_combo()
    if game_over:
        bot.send_message(user_id, f"Победил Бот!")
        bot.send_message(user_id, f"Ещё разок? /start!")
    else:
        if max_move > candy:
            max_move = candy
